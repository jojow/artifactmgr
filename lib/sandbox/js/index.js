...
var apiListener = function(data) {
    ...
    message.args.push(function() {
        var reply = { corr: message.corr, args: _.toArray(arguments) };
...
exports.run = function(jsSource, input, context, callback) {
    ...
    var script = sandcastle.createScript('exports.main = function() {\
                                              var callback = _.once(function(err, output) {\
                                                  if (err) exit({ _error: err });\
                                                  else exit({ output: output, context: context });\
                                              });\
                                              ' + jsSource + '\
    ...
    script.on('exit', function(err, result) {
        if (err) callback(err);
        else if (result && result._error) callback(result._error);
        else callback(null, result.output, result.context);
    });
