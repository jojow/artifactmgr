    ...
    'sandbox-js': function(impl, input, context, callback) {
        ...
        var execInSandbox = function(err, jsSource) {
            ...
            jsSource = '(function() {\
                            ...
                            var _appendErr = function(err, str) {\
                                if (_.isString(err)) err += str;\
                                else if (!err.message) err.message = str;\
                            ...
                            var exec = function(command, stdin, options, callback) {\
                                ...
                                if (options.sudo)\
                                    ...
                                    command = host.commands.execCmd({ dir: dir,\
                                                                      ...
                                                                      command: command });\
                                \
                                host.exec(command, stdin, options, function(err, stdout, stderr) {\
                                    if (err && options.stderr2err) err = _appendErr(err, "\\nstderr: " + stderr);\
                                    if (err && options.stdout2err) err = _appendErr(err, "\\nstdout: " + stdout);\
                                    \
...
var executeDep = function(host, run, dep, callback) {
    ...
    executeImpl(impl, input, context, function(err, output, updatedContext) {
        if (err) {
            ...
            var errContext = { id: dep.id };

            return callback(err, { error: { message: message, stack: stack, context: errContext } }, context);
        }
    ...
    async.waterfall([
        ...
        function(data, cbParallel) {
            ...
            host.init(function(err) {
                ...
                function(err) {
                    host.cleanup(function(cleanupErr) {
                        if (err) return cbParallel(err, run, runContext);
                        else if (cleanupErr) return cbParallel(cleanupErr, run, runContext);
